...
//Define motor pins
#define motorSprinkler 43
#define motorReservior 44
#define motorPesticide 45
#define mistHumidifier 42

//Define sensor pins
#define sensorPinA A4
#define sensorPinB A5
#define sensorPinC A6
#define motionPin 12
...
void setup() {
  ...
  lcd.setCursor(13, 1);
  lcd.print("A:NONE ");  //Audio Sensor
  lcd.setCursor(13, 2);
  lcd.print("V:NONE ");  //Visual Sensor
  lcd.setCursor(7, 2);
  lcd.print("R:OFF");  //Reservior Tank
  lcd.setCursor(7, 3);
  lcd.print("P:OFF");  //Pesticide
}
...
void loop() {
  ...
  if (soilMoistureA < soilWetA) {
    ...
    servoMotor.write(0);
    // ON sprinkler
    digitalWrite(motorSprinkler, HIGH);
    lcd.setCursor(7, 1);
    lcd.print("S:ON ");
  }
...
void pestDetectedMotion(void) {
  ...
  lcd.setCursor(7, 3);
  lcd.print("P:ON ");
  digitalWrite(mistHumidifier, HIGH);
  digitalWrite(motorPesticide, HIGH);
...
void emptyTank() {
  ...
  digitalWrite(motorReservior, HIGH);
  emptyState = true;
}
void fullTank() {
  // ON motor water reserved
  Serial.println("UPPER");
  digitalWrite(motorWaterRes, LOW);
  fullState = true;
}
void serialEvent() {
